...
//¼¼ºÎ ³»¿ë ÀÔ·Â
function addDetailedCalendar($st, $et, $sub, $ade, $dscr, $loc, $color, $tz, $rmdy_doct, $asin_seqn, $cust_name, $cust_tele, $cust_cnum, $cust_gubn, $rmdy_code, $clnc_code, $clnc_gubn, $oper_chck, $inet_flag, $tele_flag, $resv_memo, $re){
  $ret = array();
  try{
    $sql = "insert into `jqcalendar` (`subject`, `starttime`, `endtime`, `isalldayevent`, `description`, `location`, `color`, `CUST_NAME`, `CUST_TELE`, `CUST_CNUM`, `CUST_GUBN`, `RMDY_CODE`, `CLNC_CODE`, `RMDY_DOCT`, `ASIN_SEQN`, `RESV_YYMD`, `RSST_HHMI`, `RSED_HHMI`, `REGI_DATE`, `REGI_EMPL`, `OPER_CHCK`, `INET_FLAG`, `TELE_FLAG`, `CLNC_GUBN`, `RESV_MEMO`, `RESV_IPAD`) values ('"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
      .mysql_real_escape_string($ade)."', '"
...
function updateCustomer($cust_name, $cust_iden, $iden_chck, $cust_tele, $cust_hand, $sms_chck, $bith_year, $bith_mmdd, $bith_flag, $home_post, $offi_post, $addr_chck, $post_gubn, $sex_gubn, $mary_year, $mary_mmdd, $cust_mail, $mail_chck, $offi_addr, $home_addr, $cust_memo, $uu, $cust_cnum){
  $ret = array();
  try{
...
//¼¼ºÎ ³»¿ë ¾÷µ¥ÀÌÆ®
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $color, $tz, $rmdy_doct, $asin_seqn, $cust_name, $cust_tele, $cust_cnum, $cust_gubn, $rmdy_code, $clnc_code, $clnc_gubn, $oper_chck, $inet_flag, $tele_flag, $resv_memo, $uu){
  $ret = array();
  try{
    $sql = "update `jqcalendar` set"
      . " `starttime`='" . php2MySqlTime(js2PhpTime($st)) . "', "
...
switch ($method) {
	...
	$bith_mmdd=substr($_POST["CUST_BITH"],-5);
    	$mary_year=substr($_POST["MARY_DATE"],0,4);
	$mary_mmdd=substr($_POST["MARY_DATE"],-5);
        if($_POST["CUST_CNUM"]){		
		updateCustomer($_POST["CUST_NAME"], $_POST["CUST_IDEN"], $_POST["IDEN_CHCK"], $_POST["CUST_TELE"], $_POST["CUST_HAND"], $_POST["SMS_CHCK"], $bith_year, $bith_mmdd, $_POST["BITH_FLAG"], $_POST["HOME_POST"], $_POST["OFFI_POST"], $_POST["ADDR_CHCK"], $_POST["POST_GUBN"], $_POST["SEX_GUBN"], $mary_year, $mary_mmdd, $_POST["CUST_MAIL"], $_POST["MAIL_CHCK"], $_POST["OFFI_ADDR"], $_POST["HOME_ADDR"], $_POST["CUST_MEMO"], $re, $_POST["CUST_CNUM"]);
		}else{
		updateCustomer($_POST["OFFI_ADDR"], $_POST["HOME_ADDR"], $re, $_POST["CUST_CNUM"]);
		//addCustomer();
		}
	break;
    case "adddetails":
        ...
        if(isset($_GET["id"])){
            $ret = updateDetailedCalendar($_GET["id"], $st, $et, 
                ...
                $_POST["Location"], $_POST["colorvalue"], $_POST["timezone"], $_POST["RMDY_DOCT"], $_POST["ASIN_SEQN"], $_POST["CUST_NAME"], $_POST["CUST_TELE"], $_POST["CUST_CNUM"], $_POST["CUST_GUBN"], $_POST["RMDY_CODE"], $_POST["CLNC_CODE"], $_POST["CLNC_GUBN"], $_POST["OPER_CHCK"], $_POST["INET_FLAG"], $_POST["TELE_FLAG"], $_POST["RESV_MEMO"], $re);
        }else{
